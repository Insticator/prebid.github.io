...
| `audio.podseq`          | optional    | Sequence of the audio ad pod within the content stream | `0` |
| `audio.sequence`        | optional    | For multiple ads in the same bid request, to allow coordinated delivery. Deprecated in OpenRTB 2.6 in favour of `slotinpod` | `1` |
| `audio.slotinpod`       | optional    | Slot position in the pod the seller can guarantee delivery against | `1` |
...
| `audio.durfloors`       | optional    | Floor prices for audio creatives of various durations | `[{ mindur: 15,`<br/>`maxdur: 30,`<br/>`bidfloor: 1.5 }]` |
